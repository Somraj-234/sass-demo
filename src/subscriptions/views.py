...
def user_subscription_view(request,):
    ...
    if request.method == "POST":
        ...
        if finished:
            ...
            messages.error(request, "Your plan details has not been refreshed, please try again.")
...
def user_subscription_cancel_view(request,):
    user_sub_obj, created = UserSubscription.objects.get_or_create(user=request.user)
    # print("Subscription status:", user_sub_obj.is_active_status)
    if request.method == "POST":
        # print("refresh sub")
        if user_sub_obj.stripe_id and user_sub_obj.is_active_status:
            sub_data = helper.billing.cancel_subscription(
